package io.devground.spring_batch_prac.domain.base.genFile.service;
...
import io.devground.spring_batch_prac.domain.base.genFile.repository.GenFileRepository;
...
public class GenFileService {
	...
	public Optional<GenFile> findBy(String relTypeCode, long relId, String typeCode, String type2code, long fileNo) {
		return genFileRepository.findByRelTypeCodeAndRelIdAndTypeCodeAndType2CodeAndFileNo
			(
	...
	@Transactional
	public GenFile save(
		String relTypeCode, long relId, String typeCode, String type2Code, long fileNo, MultipartFile sourceFile
	) {
	...
	@Transactional
	public GenFile save(
		String relTypeCode, long relId, String typeCode, String type2Code, long fileNo, String sourceFilePath
	) {
		if (!Ut.file.exists(sourceFilePath)) {
			return null;
		}

		if (fileNo > 0) {
			remove(typeCode, relId, typeCode, type2Code, fileNo);
		}

		String originFileName = Ut.file.getOriginFileName(sourceFilePath);
		...
		Ut.file.moveFile(sourceFilePath, file);
		Ut.file.remove(sourceFilePath);
	...
	public Map<String, GenFile> findGenFilesMapKeyByFileNo(
		String relTypeCode, long relId, String typeCode, String type2Code
	) {
		List<GenFile> genFiles = genFileRepository
			.findByRelTypeCodeAndRelIdAndTypeCodeAndType2CodeOrderByFileNoAsc(relTypeCode, relId, typeCode, type2Code);

		return genFiles.stream()
			.collect(Collectors.toMap(
				genFile -> String.valueOf(genFile.getFileNo()),
				genFile -> genFile
			));
	}
	...
	public GenFile tempToFile(String url, BaseEntity entity, String typeCode, String type2Code, long fileNo) {
		...
		GenFile newGenFile = save(
			entity.getModelName(), entity.getId(), typeCode, type2Code, fileNo, tempGenFile.getFilePath()
		);
	...
	private long genNextFileNo(String relTypeCode, long relId, String typeCode, String type2Code) {
		return genFileRepository
			.findTop1ByRelTypeCodeAndRelIdAndTypeCodeAndType2CodeOrderByFileNoDesc(
